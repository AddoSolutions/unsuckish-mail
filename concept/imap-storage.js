...
MongoDecorator.prototype.parse_raw_msg = function(callback) {
	...
	mailparser.on('attachment', (function(attachment) {
		attachment.filePath = 'attachments/' + uuid.v4();
		attachment.stream.pipe(fs.createWriteStream(path.join(this.attachments_path, attachment.filePath)));
 ...
 * @TODO: Merge this with the core mail parser
 */
...
MongoDecorator.prototype.msgs_find = function(user, folder, flags, limit, callback) {
	var query = {};
	//console.log("  --  Finding " + folder['special-use'] || folder);
	if (user) {
		//query.user = user._id;
	}
	if (folder) {
		query.folder = folder['special-use'] || folder['path'] || folder;
	}
	if (flags) {
		query.flags = flags;
	}
...
MongoDecorator.prototype.user_has_password = function(user, remote_password, callback) {
	//console.log(remote_password);
	//console.log(user);
	if (!remote_password || !user) {
		callback(null, false);
	}else if(remote_password == user.password){
		callback(null, true);
	} else {
