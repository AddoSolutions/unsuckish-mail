...
export default class Email extends BasicModel{
	...
	static _getAttachmentsPath(){
		var path = "local/attachments";
		if(this.dataService){
			path = this.dataService.server.config.attachments.path;
		}


		// Ensure attachments path exists
		if (!this._ensuredAttachmentPath && !fs.existsSync(path)){
			fs.mkdirSync(path);
			this._ensuredAttachmentPath = true;
		}
